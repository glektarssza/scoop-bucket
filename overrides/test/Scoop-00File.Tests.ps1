...
BeforeDiscovery {
    ...
    $script:repo_files = (Get-ChildItem $TestPath -File -Recurse).FullName |
    Where-Object {
        $_ -inotmatch $($project_file_exclusions -join '|')
    };
}

Describe 'Code Syntax' -ForEach @(, $script:repo_files) -Tag 'File' {
    BeforeAll {
        $script:files = @(
...
Describe 'Style constraints for non-binary project files' -ForEach @(, $script:repo_files) -Tag 'File' {
    BeforeAll {
        $script:files = @(
